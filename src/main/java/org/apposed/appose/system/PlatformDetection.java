...
package org.apposed.appose.system;
...
public class PlatformDetection {
	...
	public static OS getOs()
	{
		...
		else if (System.getProperty("os.name").toLowerCase().contains("linux") || System.getProperty("os.name").toLowerCase().endsWith("ix")) {
			os = OS.LINUX;
		}
		else {
			throw new IllegalArgumentException(
				"Operating system not supported by Miniconda: " + System.getProperty("os.name") + ". Only supported OS are: " + OS.WINDOWS + ", " + OS.OSX + " and " + OS.LINUX);
		}
		return os;
	}
	...
	public static String getArch()
	{
		...
		if (ARCH.equals(ARCH_X86_64) && !getOs().equals(OS.WINDOWS)) {
			try {
				...
				if (txt.toLowerCase().contains("apple m")) ROSETTA = true;
			}
			catch (IOException e) {
				e.printStackTrace();
	...
	private static String executeUnameM() {
		if (UNAME_M != null) return UNAME_M;
		Process proc;
		try {
			proc = Runtime.getRuntime().exec(new String[] {"bash", "-c", "uname -m"});
	...
	public static int getJavaVersion() {
		if (JAVA_VERSION != null) return JAVA_VERSION;
